# Econometrics III - Problem Set 3 Solution
...
function mnl_loglikelihood(β, X, Z, y)
    ...
    for i in 1:n
        denom = 1.0
        for j in 1:(J-1)
            denom += exp(dot(X[i,:], β[((j-1)*k+1):(j*k)]) + β[end] * (Z[i,j] - Z[i,J]))
        end
        
        if y[i] < J
            num = exp(dot(X[i,:], β[((y[i]-1)*k+1):(y[i]*k)]) + β[end] * (Z[i,y[i]] - Z[i,J]))
        else
            num = 1.0
        end
...
function estimate_mnl(X, Z, y)
    ...
    β_init = vcat(repeat([0.0], (J-1)*k), [0.0])
