...
function allwrap()
    ...
    @testset "Econometrics Analysis Tests" begin
        ...
        @test mnl_loglikelihood(zeros((size(Z, 2) - 1) * size(X, 2) + 1), X[1:5,:], Z[1:5,:], y[1:5]) isa Float64
        
        # Test nested logit log-likelihood function
        @test nested_logit_loglikelihood(zeros(2 * size(X, 2) + 3), X[1:5,:], Z[1:5,:], y[1:5]) isa Float64
