...
==========================================================#
function load_data()
    # Correct URL for NLSY dataset under PS8-factor
    ...
    try
        ...
        if response.status == 200
            ...
            println("Successfully loaded data with $(nrow(df)) observations")
            return df
        else
            error("Failed to load data: HTTP status $(response.status)")
        end
    catch e
